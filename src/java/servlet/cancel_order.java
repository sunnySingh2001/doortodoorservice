...
public class cancel_order extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      String id_var=request.getParameter("x");  
    
      
       String sql="delete from order_items where orderid="+id_var+"";
        Statement st=con.createStatement();
        int i=st.executeUpdate(sql);
        if(i>0)
        {
             response.sendRedirect("cancel_order.jsp?status=order canceled !!!");
            //response.sendRedirect("cancel_order.jsp");
            //out.println("<h1>Row succesfully deleted</h1>" );
        }
        else
        {
          out.println("<h1>Couldn't delete</h1>" );   
        }
