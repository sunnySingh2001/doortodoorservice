...
public class delete_agent extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String agentid_var=request.getParameter("x");  
    
      
        String sql="delete from agent where agent_id="+agentid_var+"";
        Statement st=con.createStatement();
        int i=st.executeUpdate(sql);
        if(i>0)
        {
             response.sendRedirect("view_agent.jsp");
            //out.println("<h1>ITEM succesfully deleted</h1>" );
        }
        else
        {
