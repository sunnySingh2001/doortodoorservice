...
public class add_item extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      String description_var=request.getParameter("items");
      //String itemno_var=request.getParameter("itemno");  
      String category_var=request.getParameter("category");
      String unit_var=request.getParameter("unit");
      String brand_var=request.getParameter("brand");  
      String price_var=request.getParameter("price");
    
      String sql1="insert into items values(null,'"+description_var+"','"+category_var+"','"+unit_var+"','"+brand_var+"','"+price_var+"')";
        Statement st=con.createStatement();
        int i=st.executeUpdate(sql1);
        if(i>0)
        {
             response.sendRedirect("add_item.jsp?status=item added !!!");   
